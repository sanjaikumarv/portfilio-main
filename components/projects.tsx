...
import { contentFulApiCLient } from "@/lib/content-provider/contentful/client";
...
export default function Projects() {
  ...
  const getProjects = async () => {
    const response: any = await contentFulApiCLient.getEntries({
      content_type: "projects",
      include: 3, // resolve nested references
    });
    const resolveImage = (imageRefId: string) => {
      const asset = response.includes?.Asset?.find(
        (asset: any) => asset.sys.id === imageRefId
      );
      console.log("asset", "https:" + asset.fields.file.url);
      return asset ? "https:" + asset.fields.file.url : null;
    };

    const mappedProjects = response.items.map((item: any) => ({
      ...item.fields,
      imageUrl: resolveImage(item.fields.image?.sys.id),
    }));
    ...
    <div className='grid grid-cols-1 md:grid-cols-2 gap-10'>
      {[1, 2, 3, 4].map((i) => (
        <div
          ...
          <div className='relative h-64 bg-gray-300 dark:bg-gray-700'></div>
